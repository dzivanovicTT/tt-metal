...
void Run1DFabricPacketSendTest(
    ...
    for (size_t i = 0; i < line_size * 4; i++) {
        auto global_semaphores =
            tt::tt_metal::CreateGlobalSemaphore(  //) ttnn::global_semaphore::create_global_semaphore_with_same_address(
                devices_,
        ...
        auto global_semaphore_addr = global_semaphores.address();
...
tt::tt_fabric::Topology get_topology(FabricTestMode fabric_mode) {
    switch (fabric_mode) {
...
void Run1DFullMeshFabricPacketSendTest(
    ...
    per_axis_array_t<std::vector<std::vector<CoreCoord>>> worker_cores_vec_per_axis_per_device;
    std::unordered_map<size_t, std::vector<CoreCoord>> worker_cores_per_device;
    tt::tt_metal::SmallVector<std::shared_ptr<Buffer>> device_dest_buffers;
    std::unordered_map<IDevice*, Program> device_programs;
