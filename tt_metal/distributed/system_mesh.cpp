...
std::optional<MeshCoordinate> global_to_local(const MeshCoordinate& global_coord, const MeshShape& local_shape, const MeshCoordinate& local_offset) {
    tt::stl::SmallVector<uint32_t> local_coord(global_coord.dims());
    for (size_t dim = 0; dim < global_coord.dims(); ++dim) {
        if (global_coord[dim] < local_offset[dim] || global_coord[dim] >= local_offset[dim] + local_shape[dim]) {
            return std::nullopt;
        }
        local_coord[dim] = global_coord[dim] - local_offset[dim];
    }
...
public:
    ...
    virtual const MeshShape& get_shape() const = 0;
    virtual const MeshShape& local_shape() const = 0;
    virtual MeshCoordinate get_global_device_coordinate(int physical_device_id) const = 0;
    virtual std::vector<chip_id_t> get_mapped_physical_device_ids(
        const MeshShape& shape, const std::optional<MeshCoordinate>& offset = std::nullopt) const = 0;
    virtual chip_id_t get_physical_device_id(const MeshCoordinate& coord) const = 0;
    virtual uint32_t get_physical_mesh_id(const MeshCoordinate& coord) const = 0;

    virtual ~Impl() = default;
};

class SystemMesh::SingleHostImpl : public SystemMesh::Impl {
private:
...
public:
    ...
    const MeshShape& get_shape() const override;
    const MeshShape& local_shape() const override;
    MeshCoordinate get_global_device_coordinate(int physical_device_id) const override;
...
SystemMesh::SingleHostImpl::SingleHostImpl() : physical_coordinates_(get_system_mesh_coordinate_translation_map()) {
...
const MeshShape& SystemMesh::SingleHostImpl::get_shape() const { return physical_coordinates_.shape(); }

const MeshShape& SystemMesh::SingleHostImpl::local_shape() const { return physical_coordinates_.shape(); }

chip_id_t SystemMesh::SingleHostImpl::get_physical_device_id(const MeshCoordinate& coord) const {
    return physical_coordinates_.at(coord).chip_id();
}

uint32_t SystemMesh::SingleHostImpl::get_physical_mesh_id(const MeshCoordinate& coord) const {
    return *physical_coordinates_.at(coord).mesh_id();
}

MeshCoordinate SystemMesh::SingleHostImpl::get_global_device_coordinate(int physical_device_id) const {
    for (const auto& [logical_coordinate, physical_mesh_coordinate] : physical_coordinates_) {
...
std::vector<chip_id_t> SystemMesh::SingleHostImpl::get_mapped_physical_device_ids(
...
class SystemMesh::DistributedImpl : public SystemMesh::Impl {
private:
    MeshContainer<PhysicalMeshCoordinate> physical_coordinates_;
...
public:
    DistributedImpl(MeshShape global_shape, MeshCoordinate local_offset);
...
std::unique_ptr<SystemMesh::Impl> SystemMesh::Impl::create() {
    if (distributed::multihost::DistributedContext::using_mpi_environment()) {
        ...
        auto global_shape = control_plane.get_mesh_shape(tt_fabric::ControlPlaneMode::GLOBAL_MESH);
        auto local_offset = control_plane.get_local_mesh_offset();
        return std::make_unique<SystemMesh::DistributedImpl>(global_shape, local_offset);
    }
    return std::make_unique<SystemMesh::SingleHostImpl>();
}

SystemMesh::DistributedImpl::DistributedImpl(MeshShape global_shape, MeshCoordinate local_offset)
    : physical_coordinates_(get_system_mesh_coordinate_translation_map()), global_shape_(global_shape), local_offset_(local_offset) {
}

const MeshShape& SystemMesh::DistributedImpl::get_shape() const { return global_shape_; }

const MeshShape& SystemMesh::DistributedImpl::local_shape() const { return physical_coordinates_.shape(); }

chip_id_t SystemMesh::DistributedImpl::get_physical_device_id(const MeshCoordinate& coord) const {
    auto local_coord = global_to_local(coord, physical_coordinates_.shape(), local_offset_);
    if (local_coord.has_value()) {
        auto physical_device_id = physical_coordinates_.at(*local_coord).chip_id();
        log_info(LogMetal, "Translating global coordinate: {} to local coordinate: {}", coord, *local_coord);
        log_info(LogMetal, "Physical coordinates: {}", physical_coordinates_.shape());
        log_info(LogMetal, "Local offset: {}", local_offset_);
        log_info(LogMetal, "Physical device ID: {}", physical_device_id);
        log_info(LogMetal, "Global shape: {}", global_shape_);

        return physical_device_id;
    }
    TT_THROW("Coordinate {} is not in the local mesh", coord);
}


uint32_t SystemMesh::DistributedImpl::get_physical_mesh_id(const MeshCoordinate& coord) const {
    auto local_coord = global_to_local(coord, physical_coordinates_.shape(), local_offset_);
    if (local_coord.has_value()) {
...
MeshCoordinate SystemMesh::DistributedImpl::get_global_device_coordinate(int physical_device_id) const {
    for (const auto& [logical_coordinate, physical_mesh_coordinate] : physical_coordinates_) {
        if (physical_mesh_coordinate.chip_id() == physical_device_id) {
            return logical_coordinate;
        }
...
std::vector<chip_id_t> SystemMesh::DistributedImpl::get_mapped_physical_device_ids(
    const MeshShape& shape, const std::optional<MeshCoordinate>& offset) const {
    // This implementation follows the same logic as SingleHostImpl
    std::vector<chip_id_t> physical_device_ids;
...
SystemMesh::SystemMesh() : pimpl_(Impl::create()) {}
