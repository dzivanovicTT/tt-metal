...
FILE* OpenSiliconDebuggerInterfaceFile() {
    FILE* f;
    if ((f = fopen(
             (MetalContext::instance().rtoptions().get_root_dir() + SILICON_DEBUGER_RUNTIME_DATA_FILE).c_str(), "a")) ==
        nullptr) {
...
void InitSiliconDebuggerInterfaceFile() {
    ...
    fprintf(
        ...
        sizeof(mailboxes_t::launch_msg_rd_ptr));
    for (int i = 0; i < launch_msg_buffer_num_entries; i++) {
        fprintf(
            f,
            "  mailboxes_t.launch[%d].kernel_config.watcher_kernel_ids[0]:\n    offset: %ld\n    size: %ld\n",
            i,
            offsetof(mailboxes_t, launch[i].kernel_config.watcher_kernel_ids[0]),
            sizeof(mailboxes_t::launch[i].kernel_config.watcher_kernel_ids[0]));
        fprintf(
            f,
            "  mailboxes_t.launch[%d].kernel_config.watcher_kernel_ids[1]:\n    offset: %ld\n    size: %ld\n",
            i,
            offsetof(mailboxes_t, launch[i].kernel_config.watcher_kernel_ids[1]),
            sizeof(mailboxes_t::launch[i].kernel_config.watcher_kernel_ids[1]));
        fprintf(
            f,
            "  mailboxes_t.launch[%d].kernel_config.watcher_kernel_ids[2]:\n    offset: %ld\n    size: %ld\n",
            i,
            offsetof(mailboxes_t, launch[i].kernel_config.watcher_kernel_ids[2]),
            sizeof(mailboxes_t::launch[i].kernel_config.watcher_kernel_ids[2]));
    }
...
void SiliconDebuggerInterfaceLogKernel(
    std::shared_ptr<Kernel> kernel, const tt::tt_metal::JitBuildOptions& build_options) {
    std::lock_guard<std::mutex> lock(
        silicon_debugger_ifc_mutex);  // Allow for concurrent threads to log kernel information
    FILE* gdb_ifc_file = tt::tt_metal::OpenSiliconDebuggerInterfaceFile();
    fprintf(
        gdb_ifc_file,
        "  %d:\n    src: %s\n    out: %s\n",
        kernel->get_watcher_kernel_id(),
        kernel->kernel_source().name().c_str(),
        build_options.path.c_str());
    fflush(gdb_ifc_file);
    fclose(gdb_ifc_file);
}
