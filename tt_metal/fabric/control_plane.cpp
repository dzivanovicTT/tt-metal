...
void ControlPlane::generate_local_intermesh_link_table() {
    ...
    const auto& distributed_context = tt::tt_metal::MetalContext::instance().get_distributed_context();
    if (*distributed_context.size() == 1) {
        // No need to generate intermesh link tables when running a single process/hist
        // These tables are used to assign directions to ethernet links routing to remote hosts.
        return;
    }
    const auto& cluster = tt::tt_metal::MetalContext::instance().get_cluster();
