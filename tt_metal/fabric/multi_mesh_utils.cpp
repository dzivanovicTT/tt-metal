...
std::vector<uint8_t> serialize_to_bytes(const IntermeshLinkTable& intermesh_link_table) {
    ...
    for (const auto& [local_chan, remote_chan] : intermesh_link_table.intermesh_links) {
        auto local_eth_chan = tt::tt_fabric::flatbuffer::CreateEthChanDescriptor(
            builder, 
            local_chan.board_id, 
            local_chan.chan_id
        );
        
        auto remote_eth_chan = tt::tt_fabric::flatbuffer::CreateEthChanDescriptor(
            builder, 
            remote_chan.board_id, 
            remote_chan.chan_id
        );
        
        auto ethernet_link = tt::tt_fabric::flatbuffer::CreateEthernetLink(
            builder, 
            local_eth_chan, 
            remote_eth_chan
        );
    ...
    auto serialized_intermesh_link_table = tt::tt_fabric::flatbuffer::CreateIntermeshLinkTable(
        builder, 
        mesh_id, 
        ethernet_links_vector
    );
