...
struct ReceiverChannelCounterBasedResponseCreditSender {
    ...
    ReceiverChannelCounterBasedResponseCreditSender(size_t receiver_channel_index) :
        ...
        for (size_t i = 0; i < local_receiver_completion_counter_ptrs.size(); i++) {
            DPRINT << "local_receiver_completion_counter_ptrs[" << (uint32_t)i
                   << "]=" << (uint32_t)local_receiver_completion_counter_ptrs[i] << "\n";
        }

        DPRINT << "ctor receiver_channel_index=" << (uint32_t)receiver_channel_index
               << " completion_counter_ptr=" << (uint32_t)completion_counter_ptr
               << " *completion_counter_ptr=" << (uint32_t)*completion_counter_ptr
               << " ack_counter_ptr=" << (uint32_t)ack_counter_ptr << " *ack_counter_ptr=" << (uint32_t)*ack_counter_ptr
               << "\n";
    }

    FORCE_INLINE void send_completion_credit(uint8_t src_id) {
        DPRINT << "Counter based send compl ack. completion_counter_ptr = " << (uint32_t)completion_counter_ptr
               << " *completion_counter_ptr = " << (uint32_t)*completion_counter_ptr << " sent to "
               << (uint32_t)to_sender_remote_completion_counter_addrs[src_id] << "\n";
        *completion_counter_ptr++;
...
struct ReceiverChannelStreamRegisterFreeSlotsBasedCreditSender {
    ...
    FORCE_INLINE void send_completion_credit(uint8_t src_id) {
        DPRINT << "StreamRegBased send compl ack\n";
...
static_assert(
    !multi_txq_enabled ||
        std::is_same_v<ReceiverChannelResponseCreditSender, ReceiverChannelStreamRegisterFreeSlotsBasedCreditSender>,
    "ReceiverChannelResponseCreditSender must be ReceiverChannelStreamRegisterFreeSlotsBasedCreditSender");
...
struct init_receiver_channel_response_credit_senders_impl<ReceiverChannelStreamRegisterFreeSlotsBasedCreditSender> {
    ...
    static constexpr auto init()
        ...
        DPRINT << "init stream reg based credit senders\n";
        for (size_t i = 0; i < NUM_RECEIVER_CHANNELS; i++) {
            DPRINT << "init stream reg based credit senders[" << (uint32_t)i << "]\n";
...
struct init_receiver_channel_response_credit_senders_impl<ReceiverChannelCounterBasedResponseCreditSender> {
    ...
    static constexpr auto init() -> std::array<ReceiverChannelCounterBasedResponseCreditSender, NUM_RECEIVER_CHANNELS> {
        ...
        DPRINT << "init counter based credit senders\n";
        for (size_t i = 0; i < NUM_RECEIVER_CHANNELS; i++) {
            DPRINT << "init counter based credit senders[" << (uint32_t)i << "]\n";
...
struct SenderChannelFromReceiverStreamRegisterFreeSlotsBasedCreditsReceiver {
    FORCE_INLINE uint32_t get_num_unprocessed_acks_from_receiver() {
        DPRINT << "StreamRegBased get unprocessed acks. acks_received_counter_ptr = "
               << (uint32_t)acks_received_counter_ptr
               << " *acks_received_counter_ptr = " << (uint32_t)*acks_received_counter_ptr
               << " acks_received_and_processed = " << acks_received_and_processed << "\n";
...
struct SenderChannelFromReceiverCounterBasedCreditsReceiver {
    ...
    //     to_sender_packets_completed_streams(to_sender_packets_completed_streams[sender_channel_index])
    // {
    // }
    FORCE_INLINE uint32_t get_num_unprocessed_acks_from_receiver() {
    ...
    FORCE_INLINE uint32_t get_num_unprocessed_completions_from_receiver() {
        return get_ptr_val(to_sender_packets_completed_streams);
    }

    uint32_t to_sender_packets_acked_streams;
    uint32_t to_sender_packets_completed_streams;
};
...
struct init_sender_channel_from_receiver_credits_flow_controllers_impl<
    ...
    static constexpr auto init()
        ...
        for (size_t i = 0; i < NUM_SENDER_CHANNELS; i++) {
            // flow_controllers[i] = SenderChannelFromReceiverStreamRegisterFreeSlotsBasedCreditsReceiver();
            flow_controllers[i].acks_received_counter_ptr =
                reinterpret_cast<volatile uint32_t*>(to_sender_packets_acked_streams[i]);
            flow_controllers[i].completions_received_counter_ptr =
                reinterpret_cast<volatile uint32_t*>(to_sender_packets_completed_streams[i]);
        }
...
struct init_sender_channel_from_receiver_credits_flow_controllers_impl<
    ...
    static constexpr auto init()
        ...
        for (size_t i = 0; i < NUM_SENDER_CHANNELS; i++) {
            flow_controllers[i] = SenderChannelFromReceiverCounterBasedCreditsReceiver(i);
        }
        return flow_controllers;
    }
};
...
void run_sender_channel_step_impl(
    ...
    // get_ptr_val(to_sender_packets_completed_streams[sender_channel_index]);
    ...
    if constexpr (enable_first_level_ack) {
        ...
        // get_ptr_val(to_sender_packets_acked_streams[sender_channel_index]);
...
void run_receiver_channel_step_impl(
    ...
    if constexpr (!fuse_receiver_flush_and_completion_ptr) {
        ...
        if (unsent_completions) {
            ...
            receiver_channel_response_credit_sender.send_ack_credit(
...
void kernel_main() {
    ...
    if constexpr (receiver_txq_id != sender_txq_id) {
        ...
        DPRINT << "Enabling packet mode for receiver TXQ " << (uint32_t)receiver_txq_id << "\n";
