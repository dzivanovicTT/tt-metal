...
inline void compute_scaled_gain_and_gained_dL_dout_block(uint32_t col_start, uint32_t block_size) {
    // 2. Compute:
    ...
    for (uint32_t b = 0; b < block_size && (col_start + b) < Wt; ++b) {
        uint32_t col = col_start + b;
...
inline void compute_scale(uint32_t col_start, uint32_t block_size) {
    // 3. Compute:
    ...
    //
    // We will calculate scale iteratively, reducing it to a single, scalar value after each step.
    ...
    for (uint32_t b = 0; b < block_size && (col_start + b) < Wt; ++b) {
        ...
        if (col_start == 0) {
            ...
            // NOTE: Keep in mind that this is not the best idea to put everything in CB at once. L1 means only that
...
inline void compute_ms_a_and_c_by_ms_a(uint32_t col_start) {
    // Only compute and pack ms_a and c_by_ms_a for the first block in the row.
    if (col_start == 0) {
        // 4. Compute ms_a and c_by_ms_a for the current row.
        ...
        // NOTE2: RMS is bcasted and scaler is filled with the same value for all cols so we would have this output
        // value already bcasted. Not sure if we want to have like this, but it is how it is now.
...
inline void compute_rhs_block(uint32_t col_start, uint32_t block_size) {
    // 5. Compute:
    ...
    // NOTE: This function is called per-block.
    ...
    for (uint32_t b = 0; b < block_size && (col_start + b) < Wt; ++b) {
        ...
        // NOTE: We do not use compute_and_pack_mul or compute_and_pack_div here, because that would
        // unnecessarily pack and unpack intermediate results to and from CBs. Instead, we use registers
        // for the intermediate operations and only pack the final result to cb_rhs. This is more efficient
        // and avoids extra memory traffic.
        ...
        // Wait for c_by_ms_a (cb_c_by_ms_a) once per row (first col in row)
        if (col_start == 0) {
        ...
        // Pack result to cb_rhs
        tile_regs_commit();
        pack_and_push(rhs_register, cb_rhs);

        // Pop c_by_ms_a after last use in the row
        if ((col == Wt - 1) && c_by_ms_a_waited) {
            cb_pop_front(cb_c_by_ms_a, onetile);
            c_by_ms_a_waited = false;
        }
    }
}

inline void compute_dL_da_block(uint32_t col_start, uint32_t block_size) {
    // 6. Compute:
    ...
    for (uint32_t b = 0; b < block_size && (col_start + b) < Wt; ++b) {
        ...
        // We can pop_front rhs, since we do not need it anymore.
        cb_pop_front(cb_rhs, onetile);
    }
}

inline void compute_dL_dgamma_components_block(uint32_t col_start, uint32_t block_size) {
    // 7. Compute:
    ...
    for (uint32_t b = 0; b < block_size && (col_start + b) < Wt; ++b) {
        ...
        compute_and_pack_mul(
            dL_out_cb, cb_a_over_rms_a, /* tile_a */ tile_dL_out, /* tile_b */ 0, cb_dL_dgamma_components);
...
#define EVERYTHING_FITS_IN_L1 1
            ...
            compute_scaled_gain_and_gained_dL_dout_block(col, block_size);
            compute_scale(col, block_size);
            compute_ms_a_and_c_by_ms_a(col);
            compute_rhs_block(col, block_size);
            compute_dL_da_block(col, block_size);
            compute_dL_dgamma_components_block(col, block_size);

            // Pop input CBs after all uses in this block
            cb_pop_front(cb_input_idx, block_size);
            cb_pop_front(cb_gamma_idx, block_size);
            cb_pop_front(cb_dL_out_idx, block_size);

            // Pop intermediates after last use in this block
            cb_pop_front(cb_gained_dL_dout, block_size);
            cb_pop_front(cb_scale, block_size);
            cb_pop_front(cb_c_by_ms_a, 1);  // Only one tile per row
            cb_pop_front(cb_rhs, block_size);
            // cb_a_over_rms_a is popped inside compute_dL_dgamma_components_block per tile
#else
#endif
        }
    }
