...
class SentenceBERTPerformantRunner:
    def __init__(
        ...
        self.tt_att_mask_2 = self.tt_att_mask_2_host.to(device, sharded_mem_config_DRAM)
    ...
    def _capture_sentencebert_trace_2cqs(self):
        ...
        ttnn.copy_host_to_device_tensor(self.tt_att_mask_2_host, self.tt_att_mask_2, 1)
        ...
        self.runner_infra.ttnn_att_mask_2 = ttnn.to_memory_config(self.tt_att_mask_2, self.input_mem_config)
        ...
        spec_att_2 = self.runner_infra.ttnn_att_mask_2.spec
        self.op_event = ttnn.record_event(self.device, 0)
        ...
        trace_input_addr5 = self.runner_infra.ttnn_att_mask_2.buffer_address()
        ...
        self.ttnn_att_mask_2 = ttnn.allocate_tensor_on_device(spec_att_2, self.device)
        ...
        # assert trace_input_addr5 == self.ttnn_att_mask_2.buffer_address()

    def _execute_sentencebert_trace_2cqs_inference(
        self, tt_inputs_host=None, tt_tokens=None, tt_posids=None, tt_att_mask=None, tt_att_mask_2=None
    ):
        if tt_inputs_host is None:
            ...
            tt_att_mask_2 = self.tt_att_mask_2_host
        ...
        ttnn.copy_host_to_device_tensor(tt_att_mask_2, self.tt_att_mask_2, 1)
        self.write_event = ttnn.record_event(self.device, 1)
        ttnn.wait_for_event(0, self.write_event)
        ...
        self.ttnn_att_mask_2 = ttnn.reshard(self.tt_att_mask_2, self.input_mem_config, self.ttnn_att_mask_2)
        self.op_event = ttnn.record_event(self.device, 0)
    ...
    def run(self, input_ids=None, tokens=None, posids=None, att_mask=None, att_mask_2=None):
        tt_inputs_host, _, tt_tokens, tt_posids, tt_att_mask, tt_att_mask_2 = self.runner_infra.setup_l1_sharded_input(
            input_ids, tokens, posids, att_mask, att_mask_2
        )
        output = self._execute_sentencebert_trace_2cqs_inference(
            tt_inputs_host, tt_tokens, tt_posids, tt_att_mask, tt_att_mask_2
        )
        return output
