...
def evaluation(device, res, model_type, model, input_dtype, input_memory_config=None, model_name=None):
    ...
    if model_name == "vgg_unet":
        ...
        logger.info(f"IoU: {np.mean(iou_list):.4f}")
        logger.info(f"Dice Score: {np.mean(dice_list):.4f}")
        logger.info(f"Pixel Accuracy: {np.mean(acc_list):.4f}")
        logger.info(f"Precision: {np.mean(prec_list):.4f}")
        logger.info(f"Recall: {np.mean(recall_list):.4f}")
        logger.info(f"F1 Score: {np.mean(f1_list):.4f}")
