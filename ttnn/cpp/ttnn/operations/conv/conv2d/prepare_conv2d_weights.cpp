...
Tensor convert_tensor(const Tensor& input_tensor, const Fn& compute, const TensorSpec& output_spec) {
    ...
    tt::tt_metal::HostStorage converted_storage = host_storage.distributed_buffer().transform(compute);
    return Tensor(std::move(converted_storage), output_spec, input_tensor.distributed_tensor_config());
}
...
static Tensor to_folded_weight_layout(const Tensor& conv_weight_tensor, std::array<uint32_t, 2> stride) {
    ...
    auto fold_weights = [&]<typename T>(const tt::tt_metal::HostStorage& storage) {
        auto folded_storage =
            storage.distributed_buffer().transform([&](const tt::tt_metal::HostBuffer& input_host_buffer) {
                auto input_buffer = tt::tt_metal::host_buffer::get_as<T>(input_host_buffer);
    ...
    auto storage = std::get<tt::tt_metal::HostStorage>(conv_weight_tensor.storage());
    switch (dtype) {
