    ...
    const auto validatePagedCache = [](bool paged_cache,
        ...
        if (!paged_cache) {
            ...
            TT_FATAL(
                ...
                "max_num_blocks_per_seq must be less than or equal to max_num_blocks: max_num_blocks_per_seq={}, "
                "max_num_blocks={}",
                page_table.get_padded_shape()[1],
                cache_tensor.get_padded_shape()[0]);
        }
    };
