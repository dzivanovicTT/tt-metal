...
void kernel_main() {
    ...
    const uint32_t dst_width_stride = get_arg_val<uint32_t>(4);
    const uint32_t dst_height_offset_to_next = get_arg_val<uint32_t>(5);
    const uint32_t dst_offset = get_arg_val<uint32_t>(6);
    const uint32_t src_size_bytes = get_arg_val<uint32_t>(7);
    const uint32_t src_offset = get_arg_val<uint32_t>(8);
    ...
    const uint32_t num_dst_cores = get_arg_val<uint32_t>(11);
    const uint32_t src_rollover_offset = get_arg_val<uint32_t>(12);
    ...
    auto src_address = get_write_ptr(src_cb_id) + src_offset;

    uint32_t dst_noc_x = offset_x;
    uint32_t dst_noc_y = offset_y;

    for (uint32_t src_core = 0; src_core < num_dst_cores; src_core += 2) {
        auto dst_noc_address = get_noc_addr(dst_noc_x, dst_noc_y, get_read_ptr(dst_cb_id)) + dst_offset;
        noc_async_write_one_packet_set_state(dst_noc_address, stick_size_bytes);

        // Copy half of data data from src to dst
        for (uint32_t h = 0; h < height; h += stride_h) {
            for (uint32_t w = 0; w < width; w += stride_w) {
                noc_async_write_one_packet_with_state<true>(src_address, dst_noc_address);
                dst_noc_address += dst_width_stride;
                src_address += stick_size_bytes;
            }
            // skip lines to the next src line
            dst_noc_address += dst_height_offset_to_next;
        }
        src_address += src_size_bytes;  // dst_stride - one src image in dst size

        // iterate over src cores, with wrapping
        // to figure out better place for this!
        dst_noc_x += 2;
        if (dst_noc_x >= end_x) {
            dst_noc_x = start_x + dst_noc_x - end_x;
            dst_noc_y++;
            if (dst_noc_y >= end_y) {  // rollover
                dst_noc_y = start_y;
                src_address = get_write_ptr(src_cb_id) + src_rollover_offset;
            }
        }
    }
