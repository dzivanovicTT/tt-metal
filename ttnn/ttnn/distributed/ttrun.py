...
def build_mpi_command(
    ...
    variables for each rank. This approach avoids the need for wrapper
    scripts by using the -x flag to pass environment variables directly.

    The generated command structure is:
        mpirun [rankfile] [mpi_args] \
            -np 1 -x ENV1=val1 -x ENV2=val2 program [args] : \
            -np 1 -x ENV1=val3 -x ENV2=val4 program [args]
    ...
    Args:
        ...
        program: Program command and arguments to execute
...
@click.pass_context
def main(ctx, rankfile, binding, mesh_graph, dry_run, verbose, mpi_args):
    """tt-run - MPI process launcher for TT-Metal and TTNN distributed applications
